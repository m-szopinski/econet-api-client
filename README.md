...
const client = await EcoNetAPIClient({
	...
	// API bases
        appBaseUrl: 'https://<api-v2-id>.execute-api.eu-central-1.amazonaws.com/prod',
...
// REST – app API uses SigV4 (IAM), econet API passes through
...
| appBaseUrl        | string (URL)                                            | yes      | App API Gateway V2 base URL (SigV4 + `appid: "0"` header required) |
| econetBaseUrl     | string (URL)                                            | yes      | econetcloud archives API base URL |
